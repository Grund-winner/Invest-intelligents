  ...
  return (
    <div className="fixed bottom-6 right-4 z-50 flex flex-col items-end gap-3">
          ...
          <>
            ...
            {whatsappLink && (
              <motion.a
                ...
                className="flex items-center gap-2 bg-[#25D366] text-white rounded-full pl-3.5 pr-4 py-2.5 shadow-lg active:scale-95 transition-transform"
              >
            ...
            {telegramLink && (
              <motion.a
                ...
                onClick={(e) => e.stopPropagation()}
                className="flex items-center gap-2 bg-[#0088cc] text-white rounded-full pl-3.5 pr-4 py-2.5 shadow-lg active:scale-95 transition-transform"
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="white">
      ...
      <motion.button
        onClick={toggle}
        whileTap={{ scale: 0.9 }}
        className="w-14 h-14 rounded-full bg-gradient-to-br from-[#D4AF37] to-[#B8962E] shadow-xl flex items-center justify-center cursor-pointer active:scale-90 transition-transform"
      >
        {/* Pulse ring */}
        {!isExpanded && (
          <span className="absolute inset-0 rounded-full animate-ping opacity-20 bg-[#D4AF37]" style={{ animationDuration: '2.5s' }} />
        )}
        {/* Icon: chat bubbles when closed, X when open */}
        {isExpanded ? (
